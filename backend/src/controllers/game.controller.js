...
const playLimbo = async (req, res, next) => {
  try {
    ...
    const isWin = rolled >= targetMultiplier;
    let winAmount = 0;
    let payoutRatio = 0;

    if (isWin) {
      payoutRatio = targetMultiplier;
      winAmount = parseFloat((betAmount * targetMultiplier).toFixed(2));
      wallet.balance += winAmount;
      await wallet.save();
    }
    ...
    const betTxn = new Transaction({
      ...
      direction: "debit",
      prevBalance,
      postBalance: prevBalance - betAmount,
      refId: bet._id,
    ...
    if (isWin) {
      const winTxn = new Transaction({
        ...
        amount: winAmount,
        direction: "credit",
        prevBalance: prevBalance - betAmount,
        postBalance: wallet.balance,
        refId: bet._id,
        description: `Limbo win payout at ${targetMultiplier.toFixed(2)}x`,
      });
      await winTxn.save();
    }
